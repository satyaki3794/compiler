...
int main(){
	...
	if(top == NULL)	printf("\nSuccessfully parsed.\n");
	else	printf("Unsuccessfully parsed - non empty final stack.\n");


	printf("\nAST will be generated.\n");
	makeAST(root);
	printf("Successfully made AST.\n");

	for(i=0;i<stream_size;i++)
		if(lexemeStream[i].iden == ASSIGN)
			lexemeStream[i+1].iden = lexemeStream[i-1].iden = SKIP;
		else if(lexemeStream[i].iden == SEMICOLON && lexemeStream[i-1].iden == SEMICOLON)
			lexemeStream[i].iden = SKIP;
	
	//generate assembly code
	printf("\n.asm file will be created.\n");
	generateAssemblyCode();
	printf(".asm file successfully created.\n");
...
// for(i=0;i<stream_size;i++)printf("%d ", lexemeStream[i].iden);printf("\n");
// for(i=0;i<stream_size;i++)printf("%d ", arr[i]);printf("\n");

	for(i=0;i<stream_size;i++){

		int iden = lexemeStream[i].iden;
		switch(iden){

			case HELLO:		fprintf(fout, ".model tiny\n\n.data\n");
							break;

			case READ:		fprintf(fout, "\tmax%d db 20\n", arr[i]);
							fprintf(fout, "\tlen%d db ?\n", arr[i]);
							fprintf(fout, "\tstr%d db 20 dup('$'), 0AH, 0DH, '$'\n", arr[i]);
							break;

			case STRING:	fprintf(fout, "\tstr%d db \"%s$\"\n", arr[i], lexemeStream[i].value);
							strcpy(str[arr[i]], lexemeStream[i].value);
							arr[i]++;
							for(j=i+1;j<stream_size;j++)
								arr[j] = arr[j-1];
							break;

			case INT:		fprintf(fout, "\tvar%d db %s\n", arr[i], lexemeStream[i].value);
							strcpy(str[arr[i]], lexemeStream[i].value);
							arr[i]++;
							for(j=i+1;j<stream_size;j++)
								arr[j] = arr[j-1];
...
// for(i=0;i<stream_size;i++)printf("%d ", arr[i]);printf("\n");
	...
	int ifCondition = -1, elseSeen = 0;
	for(i=0;i<stream_size;i++){

		int iden = lexemeStream[i].iden;
		if(iden == SKIP)	continue;

		switch(iden){

			case READ:		
							fprintf(fout, "\tLEA DX, max%d\n", arr[i+1]-1);
							fprintf(fout, "\tMOV AH, 0Ah\n");
							fprintf(fout, "\tINT 21h\n\n");
							break;

			case WRITE :	
							if(elseSeen)	fprintf(fout, "\tLEA DX, str%d\n", elseSeen+1);
							else	fprintf(fout, "\tLEA DX, str%d\n", arr[i+1]-1);
							fprintf(fout, "\tMOV AH, 09h\n");
							fprintf(fout, "\tINT 21h\n\n");
							break;

			case BYE:		if(ifCondition != -1)	fprintf(fout, "x1:\n");
							fprintf(fout, "\n.exit\nend\n");
							break;

			case NUMBER:	if(arr[i] == 2)	fprintf(fout, "\tMOV AX, %s\n", lexemeStream[i].value);
							else if(arr[i] == 3)	fprintf(fout, "\tMOV BX, %s\n", lexemeStream[i].value);
							else if(arr[i] == 4)	fprintf(fout, "\tMOV BX, %s\n", lexemeStream[i].value);
							break;

			case IF:		
							// fprintf(fout, "\tMOV AX, %s\n", str[1]);
							// fprintf(fout, "\tMOV BX, %s\n", str[2]);
							fprintf(fout, "\tCMP AX, BX\n");
							break;

			case EQUAL:		ifCondition = 42;
							fprintf(fout, "\tJNE x2\n\n");
							break;

			case NE:		ifCondition = 43;
							fprintf(fout, "\tJE x2\n\n");
							break;

			case LS:		ifCondition = 44;
							fprintf(fout, "\tJGE x2\n\n");
							break;

			case GR:		ifCondition = 45;
							fprintf(fout, "\tJLE x2\n\n");
							break;

			case LE:		ifCondition = 46;
							fprintf(fout, "\tJG x2\n\n");
							break;

			case GE:		ifCondition = 47;
							fprintf(fout, "\tJL x2\n\n");
							break;

			case ELSE:		elseSeen = 1;
							fprintf(fout, "\tJMP x1\n");
							fprintf(fout, "x2:\n");
							break;

			case LOOP:		fprintf(fout, "\tMOV CX, 1\n");
